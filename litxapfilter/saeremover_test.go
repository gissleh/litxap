...
func TestSaeRemover(t *testing.T) {
	...
	for _, row := range table {
		t.Run(fmt.Sprintf("%s-%d-%s-%d", row.currSyllable, row.currPartIndex, row.nextSyllable, row.nextPartIndex), func(t *testing.T) {
			curr := &FilterTarget{Syllable: row.currSyllable, PartIndex: row.currPartIndex, Stressed: row.currStressed}
